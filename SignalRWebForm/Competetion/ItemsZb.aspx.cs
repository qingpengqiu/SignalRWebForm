...
public partial class Competetion_ItemsZb : PageBase
{
    ...
    {
        DataSet ds = null;
        ds = new DB6.tblEquipment_Item().GetList("idEquipment=" + id + "");
        sb.AppendLine("<div class=\"fblil\"><label>装备列表：</label></div><div class=\"fblir hoteltype\"><ul>");
        for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
        {
            sb.AppendLine("<li>" + ds.Tables[0].Rows[i]["cItemName"] + "<img width=\"50\" height=\"50\" src=\"" + ds.Tables[0].Rows[i]["cURL"] + "\"><label>" + ds.Tables[0].Rows[i]["cColor"] + "</label><label>" + ds.Tables[0].Rows[i]["cSize"] + "</label>/<label>" + ds.Tables[0].Rows[i]["iLimit"] + "</label><a href='ItemsZbAdd.aspx?sid=" + idcompete + "&itemid=" + id + "&id=" + ds.Tables[0].Rows[i]["id"] + "'> 编辑</a><br></li>");
        }
    ...
    protected void but_save_Click(object sender, EventArgs e)
    {
        ...
        {
            AlertHelper.JQAlertGo("请填写装备名称！", "", false);
            return;
        }
        ...
        for (int i = 0; i < 1000; i++)
        {
            ...
            if (Limit.ToString()!=string.Empty && Color != string.Empty && Size != string.Empty)
            {
                ...
                {
                    AlertHelper.JQAlertGo("请填写颜色！", "", false);
                    return;
                }
                if (Size == string.Empty)
                {
                    AlertHelper.JQAlertGo("请填写尺码！", "", false);
                    return;
                }
                sumn = sumn + Limit;
                numid++;
        ...
        #endregion
        int inum = 0;
        DataSet ds = new DB6.tblEquipment_Item().GetList("idEquipment=" + id + "");
        ...
        if (id == 0)
        {
            if (numid == 0)
            {
                AlertHelper.JQAlertGo("请填加款式！", "", false);
                return;
            }
            
            modl.dCreate = DateTime.Now;
            //int idceom = new DB6.tblEquipment().GetMaxId();
            int t = new DB6.tblEquipment().Add(modl);
            if (t > 0)
            {
                
                #region 获取动态加载数据

                if (0 == new DB6.tblEquipment().GetMaxId())
                {
                    dtsave(1);
                }
                else
                {
                    dtsave(new DB6.tblEquipment().GetMaxId() - 1);
                } 
                #endregion
                AlertHelper.JQAlertGo("添加成功！", "ItemsZblb.aspx?sid=" + idcompete + "", true);
                return;
            }
            else
            {
        ...
        {
            modl.id = id;
            bool tt = new DB6.tblEquipment().Update(modl);
            if (tt)
            {
                #region 获取动态加载数据
                dtsave(id);
                #endregion
                AlertHelper.JQAlertGo("修改成功！", "ItemsZblb.aspx?sid=" + idcompete + "", true);
                return;
            }
            else
            {
    ...
    {//动态添加款式        
        ...
        for (int i = 0; i < 1000; i++)
        {
            ...
            {
                DBM6.tblEquipment_Item mod = new DBM6.tblEquipment_Item();
                mod.cColor = Color;
                ...
                new DB6.tblEquipment_Item().Add(mod);
            }
        }
    }
}
...
public partial class Competetion_ItemsLyAdd : PageBase
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (Request.QueryString["itemid"] != null)
        {
            ...
            DBM6.tbltour idmt = new DB6.tbltour().GetModel(itemid);
        ...
        if (!IsPostBack)
        {
            ...
            DBM6.tbltour_Item model = new DB6.tbltour_Item().GetModel(id);
            if (model != null)
            {
    ...
    protected void but_save_Click(object sender, EventArgs e)
    {
        ...
        DBM6.tbltour hotel = new DB6.tbltour().GetModel(itemid);

        DBM6.tbltour_Item modl = new DBM6.tbltour_Item();
        ...
        modl.id = id;
        int isum = 0;
        bool cout = new DB6.tbltour_Item().Update(modl);
        if (cout)
        {
            DataSet ds = new DB6.tbltour_Item().GetList("idEquipment='" + itemid + "'");
            ...
            if (isum != 0)
            {
                DBM6.tbltour ddmodl = new DBM6.tbltour();
                ddmodl.id = itemid;
                ddmodl.iTotal = isum;
                new DB6.tbltour().Update(ddmodl);
            }
            AlertHelper.JQAlertGo("修改成功！", "ItemsLy.aspx?id=" + itemid + "&sid=" + idcompete + "", true);
            return;
        }
        else
        {

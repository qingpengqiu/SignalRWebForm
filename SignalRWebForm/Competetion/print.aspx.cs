...
public partial class Competetion_print : PageBase
{
    public int sid = 0;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (id == 0)
        {
            AlertHelper.JQAlertGo("请选择赛事！", "result.aspx?sid=" + sid + "&nav=6", false);
            return;
        }
        if (name.Trim().Length == 0)
        {
            AlertHelper.JQAlertGo("请填写名称！", "result.aspx?sid=" + sid + "&nav=6", false);
            return;
        }
        if (vers.Trim().Length == 0)
        {
            AlertHelper.JQAlertGo("请填写号码！", "result.aspx?sid=" + sid + "&nav=6", false);
            return;
        }
        if (!IsPostBack)
        {
            ...
            ltlbanner.Text = modelRace.cBanner.Trim().Length == 0 ? "" : "<img src=\"" + modelRace.cBanner + "\" height=\"256\" width=\"1200\" alt=\"\"/>";
            this.page_Title = title + "-成绩打印-益跑网赛事报名";
            ...
            if (model.iDoc != 1)
            {
                AlertHelper.JQAlertGo("该赛事没有证书哦！", "result.aspx?sid=" + sid + "&nav=6", false);
                return;
            }
            if (itype != model.istyle)
            {
                AlertHelper.JQAlertGo("赛事不匹配，请重新选择！", "result.aspx?sid=" + sid + "&nav=6", false);
                return;
            }
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                {
                    AlertHelper.JQAlertGo("您的成绩不存在！", "result.aspx?sid=" + sid + "&nav=6", false);
                    return;
                }
                ...
                if (!File.Exists(Server.MapPath(FilePath)))
                {
                    ...
                    if (ca.Length > 2)
                    {
                        ...
                        if (dtGoup.Rows.Count > 0)
                        {
                            ...
                            {
                                AlertHelper.JQAlertGo("该用户不存在！", "result.aspx?sid=" + sid + "&nav=6", false);
                                return;
                            }
                            ...
                            {
                                string NetResult = dt.Rows[0]["cNetResult"].ToString();
                            ...
                            {
                                AlertHelper.JQAlertGo("该赛事暂无证书！", "result.aspx?sid=" + sid + "&nav=6", false);
                                return;
                            }
                        }
                        else
                        {
                            AlertHelper.JQAlertGo("您查询的用户不存在！", "result.aspx?sid=" + sid + "&nav=6", false);
                            return;
                        }
                    }
                }
            ...
            {
                AlertHelper.JQAlertGo("您查询的用户证书不存在！", "result.aspx?sid=" + sid + "&nav=6", false);
                return;
            }
        }
    }
    ...
    private void DownLoadFdf34(string img, string Path, string cName, string cGroup, string iGroupRank, string iRank, string KM, string[] cResult)
    {
        ...
        try
        {
            ...
            column.Go();
            //column.SetSimpleColumn(new Phrase(cGroup, font), 800, 100, 280, 420, 16, Element.ALIGN_LEFT);//第三个数控制（小）左（大）右，第四个数字控制上下
            //column.Go();

            //column.SetSimpleColumn(new Phrase(KM, font), 800, 100, 280, 397, 16, Element.ALIGN_LEFT);//380控制上下
            //column.Go();

            //column.SetSimpleColumn(new Phrase(iRank, font), 800, 100, 280, 375, 16, Element.ALIGN_LEFT);//380控制上下
            //column.Go();

            //column.SetSimpleColumn(new Phrase(iGroupRank, font), 800, 100, 280, 350, 16, Element.ALIGN_LEFT);
            //column.Go();
    ...
    private void DownLoadFdf1(string img, string Path, string cName, string Km, string cGroup, string[] cResult)
    {
        ...
        {
            document.Close();
            AlertHelper.JQAlertGo("证书生成失败！", "result.aspx?sid=" + sid + "&nav=6", false);
            return;
        }
        document.Close();
    }

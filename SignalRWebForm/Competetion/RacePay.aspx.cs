...
public partial class Channels_Competetion_RacePay : PageBase
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            DataSet ds = bllsales.GetList(" cOrder_no='" + Request["orderid"].Trim() + "' and iduser=" + CurrentUser.uId);
            if (ds.Tables[0].Rows.Count > 0)
            {
                LabelTitle2.Text = ds.Tables[0].Rows[0]["iTypeName"].ToString().Trim();
                LabelTitle.Text = ds.Tables[0].Rows[0]["cItemName"].ToString().Trim();
                Labeltype.Text = ds.Tables[0].Rows[0]["cItemName"].ToString().Trim();
                Labelcost.Text = ds.Tables[0].Rows[0]["iCost"].ToString().Trim();

                idAlipay = ds.Tables[0].Rows[0]["cTrade_no"].ToString().Trim();

                idCompete = Convert.ToInt32(ds.Tables[0].Rows[0]["iTypeID"].ToString().Trim());
                DBM6.tblRace_Item mSignUp = new DB6.tblRace_Item().GetCompeteModel(idCompete);
                if (mSignUp != null)
                {
                    ...
                    int count = new DB6.tblOrder().GetRecordCount(" iStatus>1 and iTypeID=" + idCompete);
                    if (count >= mSignUp.iLimit)
                    {
                        AlertHelper.JQAlertGo("报名已满！", "/Channels/Competetion/RaceList.aspx", false);
                        return;
                    }
                    int state = new DB6.tblOrder().GetState(CurrentUser.uId, idCompete, 2);
                    if (0 == state)
                    {
                    ...
                    else if (1 == state)
                    {
                        if ("0" == Labelcost.Text)
                        {
                            if (TypeChangeDefault.NullToInt(ds.Tables[0].Rows[0]["iStatus"].ToString(), 0) >= 2)
                            {
                                AlertHelper.JQAlert("您已报名，请勿重复提交!", false);
                                return;
                            }
                            DBM6.tblOrder modelsales = new DBM6.tblOrder();
                            modelsales.iStatus = 2;
                            modelsales.dPayTime = DateTime.Now;
                            ...
                            try
                            {
                                ...
                                disModel.cContent = "您在《<a target=\"_blank\" href=\"/Competetion/RaceProject.aspx?idcompete=" + mSignUp.idCompete + "\">" + mSignUp.cTitle + "</a>》中报名成功！请您持续关注，谢谢！";
                                disModel.idInfo = Convert.ToInt32(ds.Tables[0].Rows[0]["idUser"].ToString().Trim());
                                disModel.idUser = 100001183;
                                disModel.cLoginname = "赛事服务";
                                ...
                                mActivity.cTitle = "在《<a target=\"_blank\" href=\"/Competetion/RaceProject.aspx?idcompete=" + mSignUp.idCompete + "\">" + mSignUp.cTitle + "</a>》中报名成功！邀请您<a target=\"_blank\" href=\"/Competetion/RaceProject.aspx?idcompete=" + mSignUp.idCompete + "\">[参加]</a>！";
                                mActivity.cBio = "";
                                mActivity.dCreate = DateTime.Now;
                                new DB2.tblActivity().Add(mActivity);
                                //报名赛事最低送10积分
                                if (ds.Tables[0].Rows[0]["iScore"].ToString() == "0")
                                {
                            ...
                            new DB6.tblAnnexLog().UpdateState(TypeChangeDefault.NullToInt(ds.Tables[0].Rows[0]["idUser"].ToString().Trim(), 0), mSignUp.idCompete ?? 0, 2);
                            int idSignUp = TypeChangeDefault.NullToInt(ds.Tables[0].Rows[0]["idUser"].ToString().Trim(), 0);
                            DBM6.tblRace_Item mSign = new DB6.tblRace_Item().GetModel(idSignUp);
                            if (mSign != null)
                            {
                                if (mSign.iType == 3)
                                {
                                    //ltlgoteam.Text = "<a href=\"/Channels/Competetion/Team.aspx?id=" + mSign.idCompete + "&p=" + idSignUp + "\" class=\"goteam\" target=\"_blank\">加入团队</a>";
                                    AlertHelper.JQAlert("报名成功!快去组队吧", true);
                                }
                                else
                                {
                                    AlertHelper.JQAlert("报名成功!", true);
                                }
                            }
                            AlertHelper.JQAlert("报名成功!", true);
                        }
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        ...
        DataTable dt = bllsales.GetList(" cOrder_no='" + Request["orderid"].Trim() + "' and iduser=" + CurrentUser.uId).Tables[0];
        if (dt.Rows.Count > 0)
        {

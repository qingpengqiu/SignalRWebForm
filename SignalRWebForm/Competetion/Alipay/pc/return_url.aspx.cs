...
/// </summary>
public partial class Competetion_Alipay_pc_return_url : System.Web.UI.Page
{
                ...
                if (Request.QueryString["trade_status"] == "TRADE_FINISHED" || Request.QueryString["trade_status"] == "TRADE_SUCCESS")
                {
                    ...
                    if (dtOrder.Rows.Count == 1)
                    {
                        ...
                        if ("1" == iStatus)
                        {
                            ...
                            //ltlpay.Text = "<a class=\"fkbuttom\">付款成功</a>";
                            Response.Redirect("/Competetion/payok.aspx?sid=" + dtOrder.Rows[0]["cFromToID"].ToString() + "&id=" + ID, true);
                        }
                        else if ("2" == iStatus)
                        { Response.Redirect("/Competetion/payok.aspx?sid=" + dtOrder.Rows[0]["cFromToID"].ToString() + "&id=" + ID, true); }
                        else
                        {
                            Response.Redirect("/Competetion/payok.aspx?sid=" + dtOrder.Rows[0]["cFromToID"].ToString() + "&id=" + ID + "&yy=付款验证失败", true);
                            //ltlpay.Text = "<a class=\"fkbuttom\">付款验证失败</a>";
                        }
                    }
                }
                else
                {
                    //Response.Redirect("/Competetion/payok.aspx?sid=300000009&id=103&yy=" + Request.QueryString["trade_status"], true);

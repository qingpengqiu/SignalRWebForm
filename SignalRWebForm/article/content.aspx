<%@ Page Language="C#" AutoEventWireup="true" CodeFile="content.aspx.cs" Inherits="article_content" %>
...
<article>
    <div class="blogleft">
        ...
        <h1><%=Title %></h1>
        <div class="bloginfo">by: <a href="/user/<%=ArticleAuthorId%>.aspx"><%=Loginname %></a>   <em><%=CreateTime %></em></div>
        ...
        <div class="share">
            ...
            <div class="blogl">阅读(<%=ShowTimes %>) 关注(<%=Focus %>) 顶(<%=Ding %>) 评论(<%=Discuss %>)</div>
        </div>
        <div class="blogpage">
            <%=Previous %><br>
            <%=Next %>
...
jQuery(document).ready(function($) {
    ...
    if (yid) {
        var ydata = $.AMUI.store.get(yid)
        $("h1").html(ydata.title)
        $(".blogshow").html(ydata.content)
        $(".bloginfo").html('<em> '+new Date().toLocaleString()+'</em>')
    };

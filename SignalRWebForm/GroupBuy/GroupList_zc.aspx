    ...
    <form id="form1" runat="server">
        ...
        <div class="wrap">
            <article>
                ...
                <div class="grouptip">
<em>商品分类</em><a href="/GroupBuy/GroupList.aspx" class="this">全部</a><a href="/GroupBuy/GroupList.aspx?id=1">装备</a><a href="/GroupBuy/GroupList.aspx?id=2">酒店</a><a  href="/GroupBuy/GroupList.aspx?id=3">营养品</a><a href="/GroupBuy/GroupList.aspx?id=4">电子产品</a><a href="/GroupBuy/GroupList.aspx?id=5">赛事服务</a>
                </div>
                <div class="grouplist">
                    <asp:Repeater ID="rptTour" runat="server">
                        <ItemTemplate>
                            <div class="groupbox">
                                ...
                                <a class="groupthum" href="/GroupBuy/Group_zc.aspx?id=<%#Eval("id").ToString()%>" target="_blank">
                                    <img src="<%#Eval("cImage").ToString()%>"></a>
                                <a class="grouptitle" href="/GroupBuy/Group_zc.aspx?id=<%#Eval("id").ToString()%>" target="_blank"><%#Getems(Eval("id").ToString())+Eval("cName").ToString()%></a>
                                <p>
                                    市场价：<em><i>&yen;</i><%#Eval("fMarketPrice").ToString()%></em><i><%#Getzk(Eval("id").ToString(),Eval("fMarketPrice").ToString()) %>折</i><br>
                                    <%#Getems(Eval("id").ToString()).Trim().Length==0?"益跑众筹：<span><i>&yen;</i>"+GetTour(Eval("id").ToString())+"</span>（最低）":" 一口价：<span><i>&yen;</i>"+GetTour(Eval("id").ToString())+"</span>"%>
                                </p>
                        ...
                        <FooterTemplate>
                            <asp:Label ID="lblEmpty" Text="众筹物品正在上架中，敬请期待……" runat="server" Visible='<%#bool.Parse((rptTour.Items.Count==0).ToString())%>' CssClass="lbEmpty"></asp:Label>
                        </FooterTemplate>
                    </asp:Repeater>
                </div>

<%@ Page Language="C#" AutoEventWireup="true" CodeFile="Cart_zc.aspx.cs" Inherits="GroupBuy_Cart" %><%@ Register Src="~/Controls/Erun_Header.ascx" TagPrefix="ERUN360" TagName="Erun_Header" %><%@ Register Src="~/Controls/Erun_Left.ascx" TagPrefix="ERUN360" TagName="Erun_Left" %><%@ Register Src="~/Controls/Erun_Footer.ascx" TagPrefix="ERUN360" TagName="Erun_Footer" %><!DOCTYPE html>
...
//显示提示框，目前三个参数(txt：要显示的文本；time：自动关闭的时间（不设置的话默认1500毫秒）；status：默认0为错误提示，1为正确提示；)
        ...
        $(".tobuy").click(function(event) {
            ...
            $.post("/action/AjaxTour.ashx",{m:'SubmitTour_zc',idGood:GetQueryString('id'),idReserve:$("#colorsize").val(),iNum:$("#datanum").val(),fScore:$("#UseIntegral:checked").val()?1:0,isAddress:isAddress,idCity:idCity,Cityname:Cityname,newaddress:newaddress,newzipcode:newzipcode,newname:newname,newtel:newtel,contbox:$("#contbox").val()},function(data){
                        var dielog
                        if(data.length > 3){
                            location.href="GetOrderInfo_zc.aspx?idorder="+data
                        }else{
                            switch(data){
                                case '0':
                                dielog = '失败'

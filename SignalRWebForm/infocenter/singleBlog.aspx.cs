...
public partial class infocenter_singleBlog : PageBase
{
    ...
    public int idPay = 0;
    public int idFoucs = 0;
    public int idGame = 0;
    public string url = string.Empty;//调整url 为手机专用
    public int idBolgUser = 0;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {
            bId = TypeChangeDefault.NullToInt(Request.QueryString["bId"].ToString(),0);
            DBM2.tblBLOG mBlog = new DB2.tblBLOG().GetModelByCache(bId);
            if (mBlog != null)
            {
                uId = mBlog.idUser;
                idBolgUser = uId;
                DBM1.tblMEMBER mUser = new DB1.tblMEMBER().GetModel(idBolgUser);
                if (mUser != null)
                {
                    imgurl = mUser.cAvatar;
                    NickName = mBlog.cLoginname;
                }
                ...
                else if (classify == "3")
                {
                    ...
                    m_nav += "赛事";
                    idGame = mBlog.idHref??0;
                    idPay = new DB4.tblRACE_SALES().GetRecordCount(" iState>=2 and idSignUp in( SELECT id FROM YP_DB_4.dbo.tblRACE_SIGNUP WHERE idCompete =" + idGame + ")");
                    idFoucs = new DB4.tblRACE_JOIN().GetRecordCount("idActivity=" + idGame);
                }
                else if (classify == "4")
                {
        ...
        if (!IsPostBack)
        {
            if (CurrentUser != null)
            {
                ...
                if (0 != bId)
                {
                    ...
                    if (mBlog_Summary != null)
                    {
                        ...
                        int i2 = new Random().Next(2, 11);//不包含11
                        mBlog_Summary.idBlog = bId;
                        mBlog_Summary.iShowTimes = mBlog_Summary.iShowTimes + i2;
                ...
                pager.RecordCount =new DB3.tblDISCUSS().GetRecordCount("");
            ...
            pager.RecordCount = new DB3.tblDISCUSS().GetRecordCount(" idInfo='" + bId + "' ", bId);
            BindDiscuss();
        }
        url = Request.RawUrl;
    }
    ...
    private void BindDiscuss()
    {
        ...
        if (cId == uId)
        {
            DiscussList = bInfo.GetDiscussList(CurrentUser.uId, bId, pageindex, pagesize);
        }
        else
        {
            DiscussList = bInfo.GetDiscussList(0, bId, pageindex, pagesize);
        }
    }
}
<%@ Page Language="C#" AutoEventWireup="true" CodeFile="ChangeMobile.aspx.cs" Inherits="infocenter_ChangeMobile" %>
...
<div class="rebox">
    ...
    <div class="refrome refi">
        <form id="form">
            <input type="hidden" value="changemobile" name="op" id="op">
            <div class="newregist">
                <input type="password" value="" name="loginpass" id="loginpass" class="ajaxve" data-op="verifyloginpass" data-get="0"><div class="Validform_checktip">请输入登录密码</div>
            </div>
            <div class="newregist">
                <input type="text" placeholder="手机号" value="" name="mobile" id="mobile" class="ajaxve" data-op="verificationmobile" data-get="0"><div class="Validform_checktip">只能使用机号</div>
            </div>
            <div class="newregist">
            ...
            <div class="newregist">
                <input type="text" name="msgCode" id="msgCode"  value=""  class="ajaxve m80" data-op="msgcode" data-get="0" placeholder="短信验证码" /><a class="getmsgcode" data-get="0" href="javascript:void(0)">获取短信校验码</a><div class="Validform_checktip"></div>
            </div>
            <div class="newregist"><a id="ensure" style="cursor:pointer">确定</a></div>
        </form>
    </div>
</div>
...
$(".ajaxve").blur(function(event) {
        ...
        if (op == 'msgcode') {
            if ($(".getmsgcode").data('get') == 1 || $(".getmsgcode").data('get') == 0) {
                ...
                $.get('/action/ChangeMobile.ashx?op=' + op + '&code=' + code + '&mobile=' + mobile, function (data) {
                    if (data.status == 1) {
        ...
        $.get('/action/ChangeMobile.ashx?op=' + op + '&code=' + code, function (data) {
        if (data.status == 1) {
            if (op == 'verificationcode') {
                $(".getmsgcode").addClass('gomsgcode').data('get', '1')
            };
...
$(".getmsgcode").click(function(event) {
    ...
    if ($(this).data("get") == 1 || $(this).data("get") == 3) {
        var tel = $("#mobile").val(),
            code = $("#txtCode").val()
        $.get('/action/ajaxsms.ashx?op=sendsms&type=changemobile&mobile='+tel+'&code='+code, function(data) {
            if (data.status == 1) {

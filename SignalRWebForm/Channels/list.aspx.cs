...
public partial class Channels_list : PageBase
{
    public int id = 0;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            if (model.cLinkUrl.Trim().Length == 0)
            {
                ltlImg.Text = "<img src=\"/Html/images/1001.jpg\">";
            }
            else
                ltlImg.Text = "<img src=\"" + model.cLinkUrl.Replace(HostInfo.imgName, HostInfo.imgCDN) + "\">";
            #region 文章列表
            string KeyArticle = "wzArticle-" + id;
            object Article = Erun360.Common.DataCache.GetCache(KeyArticle);
            if (Article == null)
            {
                DB2.tblPush bll = new DB2.tblPush();
                Article = bll.GetList(20, "idClass=" + id + " and dPush <=GETDATE()", "dPush desc").Tables[0];
                if (Article != null)
                {

...
namespace ERUN360.Entity
{
    ...
    public class ProductsUseInfo
    {
        ...
        public int idProduct
        {
            ...
            set { _idProduct = (int)value; }
        }
        // 跑步距离Km
        private double _fRunDistance = 0.0;
        public double fRunDistance
        {
            get { return _fRunDistance; }
            set { _fRunDistance = (double)value; }
        }
        // 创建时间
    ...
    public class ProductsUse : SQLHelper
    {
        // 错误信息
        ...
        // 数据库.表.名称
        private static string Database = "[YP_DB_5].[dbo].[tblPRODUCTS_USE]";
        ...
        /// 新增记录
        /// </summary>
        /// <param name="item">ProductsUseInfo</param>
        /// <return>成功返回true，失败返回false</return>
        public bool Insert(ProductsUseInfo item, out int index)
        {
            index = 0;
            StringBuilder sql = new StringBuilder();
            sql.Append("DECLARE @last_index int;");
            sql.Append("INSERT INTO " + Database + " ");
            sql.Append("(idUser,idProduct,fRunDistance) ");
            sql.Append("VALUES (");
            // 构建赋值语句组合
            StringBuilder _s = new StringBuilder();
            _s.Append("'" + item.idUser.ToString() + "',");
            _s.Append("'" + item.idProduct.ToString() + "',");
            _s.Append("'" + item.fRunDistance.ToString() + "',");
        ...
        /// <param name="item">ProductsUseInfo</param>
        /// <return>成功返回true，失败返回false</return>
        public bool Update(ProductsUseInfo item)
        {
            ...
            _s.Append("idProduct='" + item.idProduct.ToString() + "',");
            _s.Append("fRunDistance='" + item.fRunDistance.ToString() + "',");
            sql.Append(XString.UnTail(_s.ToString()));
            sql.Append(" ");
        ...
        /// <param name="item">ProductsUseInfo</param>
        /// <return>成功返回true，失败返回false</return>
        public bool UpdateBy(ProductsUseInfo item, string fieldName, string fieldValue)
        {
        ...
        /// </summary>
        /// <param name="id"></param>
        /// <return>ProductsUseInfo</return>
        public ProductsUseInfo Load(int local_id, string fieldName, string fieldValue)
        {
            if (local_id <= 0) { _err = "local_id <= 0"; return null; }
            ProductsUseInfo item = null;
            string sql = "";
            ...
            try
            {
                ...
                if (sr.Read())
                {
                    item = new ProductsUseInfo();
                    item.id = Int32.Parse(sr["id"].ToString());
                    item.idUser = Int32.Parse(sr["idUser"].ToString());
                    item.idProduct =Int32.Parse(sr["idProduct"].ToString());
                    item.fRunDistance = Double.Parse(sr["fRunDistance"].ToString());
        ...
        /// <return>ProductsUseInfo List</return>
        public List<ProductsUseInfo> PageList(int pageIndex, int pageSize, string conditionString, string orderString, out int pageCount)
        {
            // 预先赋值
            pageCount = 0;
            // 检测页号
            ...
            List<ProductsUseInfo> list = new List<ProductsUseInfo>();
            string sql = "";
            // 查询总数并计算总页数
            ...
            try
            {
                ...
                while (sr.Read())
                {
                    ProductsUseInfo item = new ProductsUseInfo();
                    item.id = Int32.Parse(sr["id"].ToString());
                    item.idUser = Int32.Parse(sr["idUser"].ToString());
                    item.idProduct = Int32.Parse(sr["idProduct"].ToString());
                    item.fRunDistance = Double.Parse(sr["fRunDistance"].ToString());

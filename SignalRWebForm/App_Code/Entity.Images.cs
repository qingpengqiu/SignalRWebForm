...
namespace ERUN360.Entity
{
    ...
    public class ImagesInfo
    {
        ...
        // 原始图片地址
        private string _cImageUrl = String.Empty;
        public string cImageUrl
        {
            get { return _cImageUrl; }
            set { _cImageUrl = XString.SqlSafe((string)value); }
        }
        // 缩略图地址
        private string _cThumbUrl = String.Empty;
        public string cThumbUrl
        {
            get { return _cThumbUrl; }
            set { _cThumbUrl = XString.SqlSafe((string)value); }
        }
        // 创建时间
    ...
    public partial class Images : SQLHelper
    {
        ...
        // 数据库.表.名称
        private static string Database = "[YP_DB_3].[dbo].[tblIMAGES]";
        ...
        /// 新增记录
        /// </summary>
        /// <param name="item">ImagesInfo</param>
        /// <return>成功返回true，失败返回false</return>
        public bool Insert(ImagesInfo item, out int index)
        {
            index = 0;
            StringBuilder sql = new StringBuilder();
            sql.Append("DECLARE @last_index int;");
            sql.Append("INSERT INTO " + Database + " ");
            sql.Append("(idUser,idInfo,cImageUrl,cThumbUrl) ");
            sql.Append("VALUES (");
            // 构建赋值语句组合
            StringBuilder _s = new StringBuilder();
            _s.Append("'" + item.idUser.ToString() + "',");
            _s.Append("'" + item.idInfo.ToString() + "',");
            _s.Append("'" + item.cImageUrl.ToString() + "',");
            _s.Append("'" + item.cThumbUrl.ToString() + "',");
        ...
        /// <param name="item">ImagesInfo</param>
        /// <return>成功返回true，失败返回false</return>
        public bool Update(ImagesInfo item)
        {
            ...
            _s.Append("cImageUrl='" + item.cImageUrl.ToString() + "',");
            _s.Append("cThumbUrl='" + item.cThumbUrl.ToString() + "',");
            sql.Append(XString.UnTail(_s.ToString()));
            sql.Append(" ");
        ...
        /// <param name="item">ImagesInfo</param>
        /// <return>成功返回true，失败返回false</return>
        public bool UpdateBy(ImagesInfo item, string fieldName, string fieldValue)
        {
        ...
        /// </summary>
        /// <param name="id"></param>
        /// <return>ImagesInfo</return>
        public ImagesInfo Load(int local_id, string fieldName, string fieldValue)
        {
            if (local_id < 0) { _err = "local_id < 0"; return null; }
            ImagesInfo item = null;
            string sql = "";
            ...
            try
            {
                ...
                if (sr.Read())
                {
                    ...
                    item.cImageUrl = sr["cImageUrl"].ToString();
                    item.cThumbUrl = sr["cThumbUrl"].ToString();
        ...
        /// <return>ImagesInfo List</return>
        public List<ImagesInfo> PageList(int pageIndex, int pageSize, string conditionString, string orderString, out int pageCount)
        {
            // 预先赋值
            pageCount = 0;
            // 检测页号
            ...
            List<ImagesInfo> list = new List<ImagesInfo>();
            string sql = "";
            // 查询总数并计算总页数
            ...
            try
            {
                ...
                while (sr.Read())
                {
                    ImagesInfo item = new ImagesInfo();
                    item.id = Int32.Parse(sr["id"].ToString());
                    item.idUser = Int32.Parse(sr["idUser"].ToString());
                    item.idInfo = Int32.Parse(sr["idInfo"].ToString());
                    item.cImageUrl = sr["cImageUrl"].ToString();

...
namespace ERUN360.Entity
{
    ...
    public class MemberFollowInfo
    {
        ...
        // 关联用户ID
        private int _idFollow = 0;
        public int idFollow
        {
            get { return _idFollow; }
            set { _idFollow = (int)value; }
        }
        // 信息ID
    ...
    public class MemberFollow : SQLHelper
    {
        // 错误信息
        ...
        // 数据库.表.名称
        private static string Database = "[YP_DB_1].[dbo].[tblMEMBER_FOLLOW]";
        ...
        /// 新增记录
        /// </summary>
        /// <param name="item">MemberFollowInfo</param>
        /// <return>成功返回true，失败返回false</return>
        public bool Insert(MemberFollowInfo item, out int index)
        {
            index = 0;
            ...
            // @@@@@@@@@@@@@@@@@@@@@@
            // 以免重复添加
            sql.Append("if exists(SELECT * FROM " + Database + " WHERE idUser=" + item.idUser + " AND idFollow=" + item.idFollow + " AND idInfo=" + item.idInfo + ")");
            sql.Append("begin");
            sql.Append("SELECT @last_index = -1;");
            sql.Append("SELECT @last_index;");
            sql.Append("return;");
            sql.Append("end");
            // @@@@@@@@@@@@@@@@@@@@@@
            sql.Append("INSERT INTO " + Database + " ");
            sql.Append("(idUser,idFollow,idInfo) ");
            sql.Append("VALUES (");
            // 构建赋值语句组合
            StringBuilder _s = new StringBuilder();
            _s.Append("'" + item.idUser.ToString() + "',");
            _s.Append("'" + item.idFollow.ToString() + "',");
        ...
        /// <param name="item">MemberFollowInfo</param>
        /// <return>成功返回true，失败返回false</return>
        public bool Update(MemberFollowInfo item)
        {
            ...
            _s.Append("idFollow='" + item.idFollow.ToString() + "',");
        ...
        /// <param name="item">MemberFollowInfo</param>
        /// <return>成功返回true，失败返回false</return>
        public bool UpdateBy(MemberFollowInfo item, string fieldName, string fieldValue)
        {
        ...
        /// </summary>
        /// <param name="id"></param>
        /// <return>MemberFollowInfo</return>
        public MemberFollowInfo Load(int local_id, string fieldName, string fieldValue)
        {
            if (local_id <= 0) { _err = "local_id <= 0"; return null; }
            MemberFollowInfo item = null;
            string sql = "";
        ...
        /// <return>MemberFollowInfo List</return>
        public List<MemberFollowInfo> PageList(int pageIndex, int pageSize, string conditionString, string orderString, out int pageCount)
        {
            // 预先赋值
            pageCount = 0;
            // 检测页号
            ...
            List<MemberFollowInfo> list = new List<MemberFollowInfo>();
            string sql = "";
            // 查询总数并计算总页数
            ...
            try
            {
                ...
                while (sr.Read())
                {
                    MemberFollowInfo item = new MemberFollowInfo();
                    item.id = Int32.Parse(sr["id"].ToString());
                    item.idUser = Int32.Parse(sr["idUser"].ToString());
                    item.idFollow = Int32.Parse(sr["idFollow"].ToString());
                    item.idInfo = Int32.Parse(sr["idInfo"].ToString());

...
namespace Com.Alipay.Wap
{
    ...
    public class Notify
    {
        ...
        {
            //初始化基础配置信息
            _partner = Com.Alipay.Wap.Config.Partner.Trim();
            _key = Com.Alipay.Wap.Config.Key.Trim();
            _private_key = Com.Alipay.Wap.Config.Private_key.Trim();
            _public_key = Com.Alipay.Wap.Config.Public_key.Trim();
            _input_charset = Com.Alipay.Wap.Config.Input_charset.Trim().ToLower();
            _sign_type = Com.Alipay.Wap.Config.Sign_type.Trim().ToUpper();
        }
        ...
        public string GetPreSignStr(Dictionary<string, string> inputPara)
        {
            ...
            sPara = Com.Alipay.Wap.Core.FilterPara(inputPara);

            //根据字母a到z的顺序把参数排序
            sPara = Com.Alipay.Wap.Core.SortPara(sPara);

            //获取待签名字符串
            string preSignStr = Com.Alipay.Wap.Core.CreateLinkString(sPara);
        ...
        public Dictionary<string, string> Decrypt(Dictionary<string, string> inputPara)
        {
            try
            {
                inputPara["notify_data"] = Com.Alipay.Wap.RSAFromPkcs8.decryptData(inputPara["notify_data"], _private_key, _input_charset);
            }
            catch (Exception e) { }
        ...
        private bool GetSignVeryfy(Dictionary<string, string> inputPara, string sign,bool isSort)
        {
            ...
            if (sign != null && sign != "")
            {
                ...
                {
                    case "MD5":
                        isSgin = Com.Alipay.Wap.AlipayMD5.Verify(preSignStr, sign, _key, _input_charset);
                        break;
                    case "RSA":
                        isSgin = Com.Alipay.Wap.RSAFromPkcs8.verify(preSignStr, sign, _public_key, _input_charset);
                        break;
                    case "0001":
                        isSgin = Com.Alipay.Wap.RSAFromPkcs8.verify(preSignStr, sign, _public_key, _input_charset);
                        break;

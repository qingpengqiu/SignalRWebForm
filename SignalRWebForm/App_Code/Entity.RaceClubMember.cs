...
namespace ERUN360.Entity
{
    ...
    public class RaceClubMemberInfo
    {
        // 编号
        private int _id = 0;
        public int id
        ...
        // 俱乐部ID
        private int _idClub = 0;
        public int idClub
        {
            get { return _idClub; }
            set { _idClub = (int)value; }
        }
        // 用户ID
        ...
        public string cLoginName
        {
            ...
            set { cLoginName = XString.SqlSafe((string)value); }
        }
        // 创建时间
    ...
    public class RaceClubMember : SQLHelper
    {
        // 错误信息
        ...
        // 数据库.表.名称
        private static string Database = "[YP_DB_4].[dbo].[tblRACE_CLUB_MEMBER]";

        public RaceClubMember() { }
        ...
        /// 新增记录
        /// </summary>
        /// <param name="item">RaceClubMemberInfo</param>
        /// <return>成功返回true，失败返回false</return>
        public bool Insert(RaceClubMemberInfo item, out int index)
        {
            index = 0;
            ...
            // @@@@@@@@@@@@@@@@@@@@@@
            // 以免重复添加
            sql.AppendLine("if exists(SELECT * FROM " + Database + " WHERE idClub=" + item.idClub + " AND idUser=" + item.idUser + ")");
            ...
            sql.Append("(idClub,idUser,cLoginName) ");
            sql.Append("VALUES (");
            // 构建赋值语句组合
            StringBuilder _s = new StringBuilder();
            _s.Append("'" + item.idClub.ToString() + "',");
        ...
        /// <param name="item">RaceClubMemberInfo</param>
        /// <return>成功返回true，失败返回false</return>
        public bool Update(RaceClubMemberInfo item)
        {
            ...
            // 构建赋值语句组合
            StringBuilder _s = new StringBuilder();
            _s.Append("idClub='" + item.idClub.ToString() + "',");
        ...
        /// <param name="item">RaceClubMemberInfo</param>
        /// <return>成功返回true，失败返回false</return>
        public bool UpdateBy(RaceClubMemberInfo item, string fieldName, string fieldValue)
        {
        ...
        /// </summary>
        /// <param name="id"></param>
        /// <return>RaceClubMemberInfo</return>
        public RaceClubMemberInfo Load(int local_id, string fieldName, string fieldValue)
        {
            if (local_id <= 0) { _err = "local_id <= 0"; return null; }
            RaceClubMemberInfo item = null;
            string sql = "";
            ...
            try
            {
                ...
                if (sr.Read())
                {
                    item = new RaceClubMemberInfo();
                    item.id = Int32.Parse(sr["id"].ToString());
                    item.idClub = Int32.Parse(sr["idClub"].ToString());
        ...
        /// <return>RaceClubMemberInfo List</return>
        public List<RaceClubMemberInfo> PageList(int pageIndex, int pageSize, string conditionString, string orderString, out int pageCount)
        {
            // 预先赋值
            pageCount = 0;
            // 检测页号
            ...
            List<RaceClubMemberInfo> list = new List<RaceClubMemberInfo>();
            string sql = "";
            // 查询总数并计算总页数
            ...
            try
            {
                ...
                while (sr.Read())
                {
                    RaceClubMemberInfo item = new RaceClubMemberInfo();
                    item.id = Int32.Parse(sr["id"].ToString());
                    item.idClub =Int32.Parse(sr["idClub"].ToString());
                    item.idUser = Int32.Parse(sr["idUser"].ToString());
                    item.cLoginName = sr["cLoginName"].ToString();
